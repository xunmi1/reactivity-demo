...
it('单个 `effect`', () => {
  ...
  const mock = vi.fn().mockImplementation(() => target.a);
  effect(mock);
...
it('清理脏依赖', () => {
  ...
  const mock = vi.fn().mockImplementation(() => target.a && target.b);
  effect(mock);
  expect(mock).toHaveBeenCalledTimes(1);
  target.a = false;
  expect(mock).toHaveBeenCalledTimes(2);
...
it('嵌套 `effect`', () => {
  ...
  const mockNestEffectFn = vi.fn().mockImplementation(() => target.b);
  const mockEffectFn = vi.fn().mockImplementation(() => {
